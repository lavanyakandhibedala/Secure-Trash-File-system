...
static int my_sgfs_unlink(struct inode *dir, struct dentry *dentry)
{
        int err;
        struct dentry *lower_dentry;
...
//	gettimeofday(&tv); 
  //      printk("\n time stamp : %ld\n ",tv.tv_sec);
      strcpy(outfilename,"/mnt/sgfs/.sg/"); 
      strcat(outfilename, dentry->d_name.name );
      strcat(outfilename,".enc");
      key_data = (char *) dentry -> d_sb -> s_root -> d_inode-> i_private;
printk("$$$$$$$$$$$$$$$$$$$$$ outfilename : %s \n ", outfilename);
       printk(" raw_data : %s \n",key_data); 
printk("$$$$$$$$$$$$$$$$$$$$$ outfilename : %s \n ", outfilename);
...
//printk("$$$$$$$$$$$$$$$$$$$$$ outfilename : %s \n ", outfilename);
        ...
        out_file = filp_open(outfilename ,O_CREAT|O_WRONLY|O_RDONLY, 0644);
