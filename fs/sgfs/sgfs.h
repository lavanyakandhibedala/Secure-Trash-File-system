...
#include </usr/src/hw2-lkandhibedal/hw2/sgctl.h>
...
extern int read_decrypt_file(struct file *f, struct file *out_f, int page_size, int flag);

//static inline int encrypt_decrypt_file(struct file *out_file, char *buffer, int file_size, char *key, bool flags);

//static inline int write_file(struct file *flip, unsigned  int len,unsigned char *data, unsigned long long  mode);

extern int read_file(struct file *flip, unsigned  int len,unsigned char *data, unsigned long long  mode);
